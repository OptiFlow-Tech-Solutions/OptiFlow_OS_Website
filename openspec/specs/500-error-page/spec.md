...
Provide a polished 500 Internal Server Error page at `/500/` that follows the site's shared error page pattern — matching the 404 page in structure, dark mode support, responsiveness, and contact information visibility. CSS for error pages is centralized in `core.css` rather than duplicated inline. Serves users who encounter server errors with clear next steps and company contact details.
