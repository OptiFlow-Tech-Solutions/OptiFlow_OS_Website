...
Define semantic color token roles for the two green accent colors in the OptiFlow design system: Success Green (`--green`) for completion/done states and Growth Green (`--lime`) for KPIs/progress indicators. Ensure all 16 marketing pages use these tokens consistently without page-level overrides.
