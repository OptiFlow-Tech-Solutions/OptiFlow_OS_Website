...
Page-specific CSS for the Home page injected via a HomeStyles component following the GlobalStyles.tsx pattern. Includes all styles from the original home.html `<style>` block: hero gradients, dashboard mockup, floating widgets, typewriter cursor, card tilt, exit overlay, WhatsApp float, and responsive/dark mode overrides.
