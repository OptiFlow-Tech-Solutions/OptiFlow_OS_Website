...
Full Feature Showcase page in the React SPA — hero with EcosystemHub, sticky FeatureNav with 6 tabs, 6 before/after transformation sections (problem + cost pill → 2 solutions → 3 metrics → dual device preview → CTA), and bottom CTA section. Content matches static HTML `src/pages/feature-showcase.html`.
