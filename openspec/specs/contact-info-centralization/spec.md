...
Enforce that all contact information (email, phone, WhatsApp, location) in source HTML files uses `{{PLACEHOLDER}}` syntax from `site.json` — the single source of truth. Prevent hardcoded contact values that would create inconsistency when details change.
